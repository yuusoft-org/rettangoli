...
async function main() {
  try {
    ...
    // Find common files by their relative paths
    const commonPaths = artifactsRelativePaths.filter((relPath) =>
      goldRelativePaths.includes(relPath)
    );
    ...
    for (const relativePath of commonPaths) {
      ...
      const { similarity, error } = await compareImages(artifactPath, goldPath);
    ...
    const mismatchingItems = results
      .filter((result) => result.similarity < 1)
      .map((result) => {
        return {
          artifactPath: result.artifactPath.replace("viz/_site/", ""),
          goldPath: result.goldPath.replace("viz/", ""),
