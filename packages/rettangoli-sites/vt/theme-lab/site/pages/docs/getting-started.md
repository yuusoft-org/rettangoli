...
If you are validating the VT theme lab, run from `packages/rettangoli-sites` and point `--root-dir` to `./vt/theme-lab/site`.
...
Generated HTML and copied assets are written to `_site/` by default.
...
- rebuild and inspect generated HTML under `_site/`
