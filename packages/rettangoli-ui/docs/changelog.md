...
- `rtgl-dialog`: added reactive `p`, `ph`, and `pv` content padding across `none|xs|sm|md|lg|xl`, with `md` as the default; `p="none"` retains its full-bleed horizontal-gutter behavior.
