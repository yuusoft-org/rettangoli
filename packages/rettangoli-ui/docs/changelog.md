...
- `rtgl-text`: runtime-managed width, flex, truncation, and wrapping styles now live in a private shadow stylesheet so updating a consumer-owned inline `style` no longer clears primitive attributes such as `w`.
