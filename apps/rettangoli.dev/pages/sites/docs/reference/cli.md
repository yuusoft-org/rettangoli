...
- Built-in page templates like `landing-features`, `blog-article-list`, and `blog-article` are **import aliases** configured in `sites.config.yaml` from `@rettangoli/sitekit`, not `init` template names.
